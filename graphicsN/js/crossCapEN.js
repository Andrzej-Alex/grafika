...
function initGui() {
    let gui = new dat.GUI();
    gui.addColor(controls, 'color1').name('disk').onChange(updateColors);
    gui.addColor(controls, 'color2').name('Mobius band').onChange(updateColors);
    let surfaces = ['disk', 'Mobius band'];
...
function updateCrossCap() {
    ...
    matArgs.transparent = surface != 'disk';
    mats = [];
