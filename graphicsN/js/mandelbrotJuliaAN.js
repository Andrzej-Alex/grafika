...
function makeMandelbrotLandscape(limit, zscale=1.0) {
    ...
    let mat = new THREE.ShaderMaterial(makeShaderMaterialArgs(frame, colors, limit));
      ...
      precision highp float;
        ...
        void main() {
            if (gl_FrontFacing) {
                ...
                float jminr = -1.5;
                float jmini = -1.5;
                float cr = ${width} * 0.5 + ${minr};
                float ci = ${width} * 0.5 + ${mini};
                vec2 c = vec2(cr, ci);
                float posr = jwidth * vUv.x + jminr;
                float posi = jheight * vUv.y + jmini;
                vec2 pos = vec2(posr, posi);
                gl_FragColor = julia(pos, c);
            }
        }
      `,
...
function recomputeFrame() {
    ...
    frameSquare.position.setZ(0.01);
    updateShader();
}
