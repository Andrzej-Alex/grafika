...
function createScene() {
    ...
    fractal = makeCornerConnected(2, new THREE.TetrahedronGeometry(len), 0.5);
...
var controls = new function() {
    ...
    this.color = '#3366ff';
    this.scale = 0.5;
    this.Stellate = function () { update(true) };
...
function initGui() {
    ...
    let typeItem = gui.add(controls, 'shape', objectTypes).onChange(update);
...
let maxNbrStellateVertices = 4;
let stellateMin = 0.1, stellateMax = 2.0;

function update(stellate=false) {
    if (fractal)
    ...
    if (stellate == true) {
        let nbrStellateVertices = MyUtils.getRandomInt(1, maxNbrStellateVertices);
